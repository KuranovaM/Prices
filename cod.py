...
for i in items:
    if i not in d_items:
        ...
        for ii in items[i]:
            ...
            for iii in items[i]:
                new_d = []
                d_items[i][ii]["максимальное время поставки"] = max(list(filter(lambda x: x is not None, items[i][ii]['время поставки'])))
                d_items[i][ii]["минимальное время поставки"] = min(list(filter(lambda x: x is not None, items[i][ii]['время поставки'])))
                ...
                #years = {}
                #for iiii in items[i][ii][iii]:
                #    if iiii not in years:
                #        years[iiii[:4]] =
print(d_items)