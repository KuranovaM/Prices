...
wb = load_workbook(filename = 'datamon.xlsx')
    ...
    if item not in d_items:
        ...
        for manufacturer in new_items[item]:
            if manufacturer not in d_items[item]:
                d_items[item][manufacturer.get_code()] = {"максимальное время поставки": 0, "минимальное время поставки": 0, "среднее время поставки": 0, "среднее изменеение цены": 0, "последняя цена": 0, "предположительная цена": 0}
            if len(list(filter(lambda x: x is not None, manufacturer.get_arrival_times()))) == 0:
                ...
                d_items[item][manufacturer.get_code()]["минимальное время поставки"] = min(
                    list(filter(lambda x: x is not None, manufacturer.get_arrival_times())))
            d_items[item][manufacturer.get_code()]["последняя цена"] = manufacturer.get_price_by_index(-1)
            pr = []
            for i in range(len(list(manufacturer.get_prices())) - 1):
                pr.append((manufacturer.get_prices()[list(manufacturer.get_prices())[i + 1]] * 100) / manufacturer.get_prices()[list(manufacturer.get_prices())[i]] - 100)
            if len(pr) == 0:
                d_items[item][manufacturer.get_code()]["среднее изменеение цены"] = 0
            else:
                d_items[item][manufacturer.get_code()]["среднее изменеение цены"] = sum(pr) // len(pr)
            d_items[item][manufacturer.get_code()]["предположительная цена"] = d_items[item][manufacturer.get_code()]["последняя цена"] + d_items[item][manufacturer.get_code()]["последняя цена"] // 100 * d_items[item][manufacturer.get_code()]["среднее изменеение цены"]
print(d_items)
prise = {}
for i in d_items:
    for ii in d_items[i]:
        infl = 8.39 / 100
        print(f"изделие {i} завод: {ii}, цена на 2022: {d_items[i][ii]['предположительная цена'] * (1 + infl) ** 1}; цена на 2023: {d_items[i][ii]['предположительная цена'] * (1 + infl) ** 2}; цена на 2024: {d_items[i][ii]['предположительная цена'] * (1 + infl) ** 3}")
        if i in prise:
            prise[i].append(d_items[i][ii]['предположительная цена'] * (1 + infl) ** 2)
        else:
            prise[i] = [d_items[i][ii]['предположительная цена'] * (1 + infl) ** 2]
for i in prise:
    print(f"{i} - цена на 2023: {sum(prise[i]) / len(prise[i])}")
